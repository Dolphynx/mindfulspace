 ...
 *
 * L’intégration SoundCloud est déléguée à {@link SoundCloudResolverService}.
 */
@Injectable()
export class MeditationSessionService {
    constructor(
        private readonly prisma: PrismaService,
        private readonly soundCloudResolver: SoundCloudResolverService,
    ) {}
        ...
        const resolved = await Promise.all(
            items.map(async (c) => {
                ...
                if (!mediaUrl && c.soundcloudUrl) {
                    mediaUrl =
                        (await this.soundCloudResolver.resolveStreamUrl(c.soundcloudUrl)) ??
                        null;
                }
