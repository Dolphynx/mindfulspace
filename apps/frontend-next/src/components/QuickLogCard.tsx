...
export default function QuickLogCard() {
    ...
    const t = useTranslations("quickLogCard");
    ...
    // ouverture automatique du date picker natif
    useEffect(() => {
    ...
    // chargement des types de session
    useEffect(() => {
        ...
        <section className="bg-white border border-brandBorder rounded-xl shadow-sm flex flex-col p-5 max-w-md w-full">
            ...
            <div className="flex items-center gap-2 mb-4">
                {(["yesterday", "today"] as const).map((choice) => (
                    ...
                    >
                        {choice === "today" ? t("today") : t("yesterday")}
                    </button>
                ))}
                ...
                <div className="relative">
                    <button
                        ...
                        aria-label={t("chooseAnotherDate")}
                        title={t("chooseAnotherDate")}
                    >
            ...
            <div className="mb-4">
                <label className="block text-gray-700 text-sm mb-2">
                    {selectedType
                        ? `${selectedType.name} (${
                            selectedType.units?.[0]?.sessionUnit.value ??
                            ""
                        }) : ${value}`
                        : `${t("valueLabel")}: ${value}`}
                </label>
