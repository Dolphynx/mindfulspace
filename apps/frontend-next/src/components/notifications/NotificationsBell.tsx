...
function formatLine(
    ...
    const target = n.requestType === "PREMIUM" ? t("target.premium") : t("target.coach");
    ...
    const tier = n.requestType === "COACH" && n.coachTier ? t("format.tier").replace("{tier}", n.coachTier) : "";
    ...
    return (
        <div className="relative">
            <button
                ...
                onClick={() => setOpen((v) => !v)}
                className={[
                    "relative rounded-lg border border-brandBorder bg-white hover:bg-brandSurface",
                    "px-2 py-1.5",
                    "lg:px-3 lg:py-2",
                ].join(" ")}
            ...
            >
                <Image
                    ...
                    width={24}
                    height={24}
                    className="block h-5 w-5 lg:h-6 lg:w-6"
                    priority
                />
                ...
                {unreadCount > 0 && (
                    <span
                        className={[
                            "absolute rounded-full bg-brandGreen text-white text-center leading-none",
                            "-top-1.5 -right-1.5 min-w-[1.1rem] px-1 py-0.5 text-[10px]",
                            "lg:-top-2 lg:-right-2 lg:min-w-[1.25rem] lg:px-1.5 lg:py-0.5 lg:text-xs",
                        ].join(" ")}
                    >
            {unreadCount}
          </span>
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-80 rounded-xl border border-brandBorder bg-white text-slate-900 shadow-lg p-2 z-50">
                    ...
                    {items.length === 0 ? (
                        <div className="px-3 py-3 text-sm text-slate-600">{t("empty")}</div>
                    ) : (
                        <div className="flex flex-col gap-1">
                            {items.map((n) => (
                                ...
                                >
                                    <div className="text-sm font-medium text-slate-900">{formatLine(n, t)}</div>
                                    <div className="text-xs text-slate-600">{t("detailsInProfile")}</div>
                                </button>
                            ))}
                        </div>
                    )}
