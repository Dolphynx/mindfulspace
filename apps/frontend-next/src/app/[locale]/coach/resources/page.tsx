...
/**
 * Coach Resources List Page
 * Displays all resources created by the current coach
 */
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link
          href={`/${locale}/coach/resources/new`}
      ...
      {filteredResources.length === 0 && !loading && (
        <div className="rounded-lg border-2 border-dashed border-brandBorder bg-brandSurface/30 p-12 text-center">
          ...
          {!searchQuery && (
            <Link
              href={`/${locale}/coach/resources/new`}
              className="mt-4 inline-block rounded-lg bg-brandGreen px-6 py-2 font-medium text-white transition hover:bg-brandGreen/90"
            >
              {t('createResource')}
            </Link>
          )}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredResources.map((resource) => (
            ...
            >
              ...
              {/* Meta */}
              <div className="mb-4 flex items-center gap-4 text-xs text-brandText/60">
                {resource.category && (
                  <span>
                ...
                {resource.readTimeMin && (
                  <span>{resource.readTimeMin} min</span>
                )}
              </div>
              ...
              <div className="flex gap-2">
                <Link
                  href={`/${locale}/coach/resources/${resource.id}/edit`}
              ...
              {deleteConfirmId === resource.id && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                  <div className="w-full max-w-md rounded-lg bg-white p-6">
                    ...
                    {resource.meditationProgramId && (
                      <p className="mt-2 text-sm text-orange-600">
                        {t('deleteConfirm.warningLinked')}
                      </p>
                    )}
