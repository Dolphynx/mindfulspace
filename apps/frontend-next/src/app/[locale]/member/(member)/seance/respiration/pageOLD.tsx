...
export default function RespirationPage() {
    ...
    useEffect(() => {
        ...
        const totalMs =
            sequence.current.reduce((sum, s) => sum + s.d, 0) * totalCycles;
        localStorage.setItem("breathingDurationSeconds", String(Math.round(totalMs / 1000)));
        localStorage.setItem("breathingStartedAt", new Date().toISOString());
