    ...
    <main className="min-h-screen flex items-center justify-center p-8">
      <div className="max-w-lg w-full rounded-2xl shadow p-6 border">
        ...
        <div className="text-sm mb-4">
          <div><span className="font-semibold">API cible :</span> {process.env.NEXT_PUBLIC_API_URL ?? '(non défini)'}</div>
          <div><span className="font-semibold">Endpoint testé :</span> <code>/health</code></div>
        </div>
        ...
        {result.ok ? (
          ...
          <div className="rounded-md border bg-red-50 p-4">
            ...
            <ul className="text-xs mt-2 list-disc pl-4">
              ...
              <li>Vérifie <code>frontend/.env.local</code> (clé <code>NEXT_PUBLIC_API_URL</code>).</li>
              <li>Vérifie que le CORS est bien activé dans l’API (voir <code>api/src/main.ts</code>).</li>
            </ul>
          </div>
        )}
      </div>
